...
class CrawlRunner:
    """
    - Điều phối một lượt thu thập dữ liệu (crawl) từ các tùy chọn đã phân giải cho đến giai đoạn xuất bản thành phẩm (artifact).
    - Một thực thể runner được gắn chặt với một source adapter, đồng thời kết hợp các bộ xử lý thu nạp (acquisition), thẻ (card), 
    chi tiết trì hoãn (deferred-detail) và danh sách mục tiêu (frontier).
    - Khi thực thi, nó sẽ thực hiện việc thu nạp qua mạng và ghi dữ liệu/trạng thái cục bộ; MySQL và Airflow nằm ngoài phạm vi này.
    """
    ...
    def __init__(
        ...
        """
        - Khởi tạo/kết hợp các phụ thuộc lúc thực thi (runtime dependencies) cho URL hoặc source adapter được cung cấp.
        - Raises:
        - ValueError: Nếu không thể phân giải (tìm thấy) adapter phù hợp cho mục tiêu được yêu cầu.
        """
        ...
        self.detail_refresh_policy = DetailRefreshPolicy(
            default_ttl_hours=self.settings.detail_refresh_ttl_hours
        )
    ...
    async def run(
        ...
        if plan:
            ...
            normalized = self.adapter.base_url.lower().strip().rstrip("/")
            target_id = hashlib.sha256(normalized.encode()).hexdigest()[:16]
        ...
        try:
            ...
            logger.exception("Crawl run failed: run_id=%s source=%s target=%s", run_id, self.adapter.SOURCE_NAME, target_id)
            
            # Save failed manifest
            finished_at = datetime.now(timezone.utc).isoformat()
            failed_result = CrawlRunResult(
                run_id=run_id,
                source=self.adapter.SOURCE_NAME,
                target_id=target_id,
                mode=getattr(options, 'mode', 'UNKNOWN') if 'options' in locals() else 'UNKNOWN',
                target_url=self.adapter.base_url,
                started_at=started_at,
                finished_at=finished_at,
                ...
                max_pages=getattr(options, 'max_pages', 0) if 'options' in locals() else 0,
                max_records=getattr(options, 'max_records', 0) if 'options' in locals() else 0,
                crawl_details=getattr(options, 'crawl_details', False) if 'options' in locals() else False,
                errors=[f"Crash: {type(exc).__name__} - {str(exc)}"]
            )
            self.storage_writer.save_manifest(failed_result)
            raise
        finally:
    ...
    async def _run_async(self, run_id: str, started_at: str, start_time: float, target_id: str, mode: str, effective_max_pages: int, effective_max_records: int, crawl_details: bool, max_details_per_run: int, stop_after_known_pages: int, start_page: int=1) -> tuple[list[RentalBronzeRecord], CrawlRunResult]:
        ...
        state = CrawlSessionState(
            ...
            is_forward_only=mode in (CrawlMode.FORWARD_ONLY_INCREMENTAL.value, 'FORWARD_ONLY_INCREMENTAL'),
        )
        now = datetime.now(timezone.utc)
        ...
        state.deferred_backlog_before = getattr(self.deferred_repository, "count_backlog", lambda s, t: 0)(self.adapter.SOURCE_NAME, target_id)

        # DISCOVERY PHASE
        state.discovery_started_at = time.perf_counter()
        ...
        while state.current_page <= state.effective_end_page:
            ...
            if acquisition_decision.should_stop:
                ...
                state.loop_exit_transition = acquisition_decision.transition
                break
                
            fetched_page_records_count = len(cards)
            ...
            frontier_decision = self.frontier_decision_processor.after_cards(
                ...
                stop_after_known_pages=stop_after_known_pages,
                raw_html=raw_html,
                record_cap_truncated_page=(
                    page_records_count < fetched_page_records_count
                ),
            )
            state.frontier_seconds += time.perf_counter() - frontier_started
            ...
            if frontier_decision.should_stop:
                state.loop_exit_transition = frontier_decision.transition
                break
                
            # ``FrontierDecisionProcessor`` owns the NEXT_PAGE transition.
            if state.current_page <= previous_page:
                raise RuntimeError(f"Crawl invariant failed: current_page ({state.current_page}) <= previous_page ({previous_page})")

        else:
            # Loop ended naturally
            from roombeacon_crawler.application.crawl.frontier_decision import FrontierTransition
            state.loop_exit_transition = FrontierTransition.PAGE_RANGE_EXHAUSTED
            
        frontier_started = time.perf_counter()
        self.frontier_decision_processor.finalize(state, state.loop_exit_transition)
        state.frontier_seconds += time.perf_counter() - frontier_started
        
        state.discovery_finished_at = time.perf_counter()
        state.discovery_seconds = state.discovery_finished_at - state.discovery_started_at

        # ENRICHMENT PHASE (Deferred)
        if getattr(state.loop_exit_transition, 'allow_deferred_enrichment', True):
            ...
            maximum_output_records = effective_max_records + deferred.attempted
            if len(state.bronze_records) > maximum_output_records:
                raise RuntimeError(
                    "Crawl output exceeded the independent discovery and "
                    "enrichment safety bounds: "
                    f"{len(state.bronze_records)} > {maximum_output_records}"
                )
        ...
        # Then update state repository if we wrote successfully (or if we intentionally had nothing to write)
        if state.updated_seen_meta and (bronze_dir or len(state.bronze_records) == 0):
            self.state_repository.record_seen_details(self.adapter.SOURCE_NAME, target_id, state.updated_seen_meta)
        ...
        deferred_remaining = self.deferred_repository.count_backlog(self.adapter.SOURCE_NAME, target_id)
        ...
        # Final validation
        if state.final_status is None or state.stop_reason is None:
             raise RuntimeError("Frontier completed without final_status or stop_reason")
    ...
    def _build_run_result(
        ...
        *,
        ...
        """Build the immutable run report from finalized session metrics."""
        return CrawlRunResult(
            ...
            pages_success=state.pages_success,
            pages_failed=state.pages_failed,
            details_success=state.detail_succeeded + state.deferred_succeeded,
            details_failed=state.detail_failed + state.deferred_failed,
            detail_requests_skipped=state.detail_skipped,
            detail_requests_forced_by_change=(
                state.detail_requests_forced_by_change
            ),
            detail_required=state.detail_required,
            detail_requested=state.detail_requested,
            detail_succeeded=state.detail_succeeded,
            detail_failed=state.detail_failed,
            detail_skipped=state.detail_skipped,
            skipped_known_unchanged_ttl=state.skipped_known_unchanged_ttl,
            skipped_no_detail_url=state.skipped_no_detail_url,
            skipped_request_budget=state.skipped_request_budget,
            skipped_deferred_for_discovery=state.skipped_deferred_for_discovery,
            skipped_source_policy=state.skipped_source_policy,
            skipped_other=state.skipped_other,
            full_address_present=state.full_address_present,
            full_address_missing=state.full_address_missing,
            coarse_only_address=state.coarse_only_address,
            detail_address_extracted=state.detail_address_extracted,
            detail_address_parse_failed=state.detail_address_parse_failed,
            deferred_backlog_before=state.deferred_backlog_before,
            deferred_added=state.deferred_added,
            deferred_attempted=state.deferred_attempted,
            deferred_succeeded=state.deferred_succeeded,
            deferred_failed=state.deferred_failed,
            ...
            records_created=len(state.new_listing_ids),
            observations_written=len(state.bronze_records),
            duplicates_skipped=state.duplicates_skipped,
            records_seen=records_seen,
            records_new=records_new,
            records_known=records_known,
            records_changed=state.records_changed,
            known_pages_streak_at_stop=state.known_page_streak,
            bootstrap_completed=state.bootstrap_completed,
            bootstrap_start_page=start_page if state.is_bootstrap else 1,
            bootstrap_next_page=state.bootstrap_next_page,
            observed_listing_ids=state.observed_listing_ids,
            new_listing_ids=state.new_listing_ids,
            errors=state.errors,
            page_acquisition_seconds=state.page_acquisition_seconds,
            listing_parse_seconds=state.listing_parse_seconds,
            card_processing_seconds=state.card_processing_seconds,
            detail_fetch_seconds=state.detail_fetch_seconds,
            deferred_detail_seconds=state.deferred_detail_seconds,
            frontier_seconds=state.frontier_seconds,
            discovery_seconds=state.discovery_seconds,
            http_client_count=self.http_fetcher.client_count,
            browser_launch_count=self.browser_fetcher.launch_count,
            browser_context_count=self.browser_fetcher.context_count,
            browser_page_count=self.browser_fetcher.page_count,
        )
